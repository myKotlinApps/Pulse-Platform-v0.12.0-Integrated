...
check("AdminKit is the only Bootstrap distribution", 'bootstrap.min.css' not in HTML)
check("Sandstone compatibility stylesheet is not loaded", 'ecu-pulse-sandstone.css' not in HTML)
check("Sandstone compatibility stylesheet is removed", not (ROOT / "assets/adminkit/css/ecu-pulse-sandstone.css").exists())
